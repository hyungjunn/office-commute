...
`ApiConvertor.getHolidays()`가 응답 내용을 검증하지 않아, 불완전하거나 오류 코드가 담긴
응답을 공휴일 0개로 계산하고 로그에 "호출 성공"을 남길 수 있었다.
→ 소정근로일 과대 → 소정근로시간 과대 → **전 직원 초과근무 과소 → 임금 미지급**.

게이트웨이 인증류 오류는 실측상 상태 코드가 붙어서 온다(잘못된 키 403, 키 누락 401).
이 경우 RestTemplate이 예외를 던져 fail-closed하며, 응답 본문 검증은 정상 응답 구조의
필수 필드와 건수 계약이 깨졌을 때 잘못된 계산으로 진행하지 않기 위한 방어선이다.
