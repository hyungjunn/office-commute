...
- **초과 근무 정산 및 리포트** (매니저 전용)
  - 초과 근무 시간 자동 계산 (근로기준법 기준): 1주(월~일) 단위로 1일 8시간 초과분과 주 40시간(실근로 기준) 초과 잔여분을 연장근로로, 일요일·공휴일 근무는 휴일근로(8시간 이내 1.5배 / 초과 2.0배)로 분리해 분 단위로 산출합니다 (근무 기록이 없는 직원은 0분으로 포함). 월 경계에 걸친 주는 일별 초과분을 해당 일의 달에, 주 40시간 잔여분을 주가 끝나는 달에 귀속합니다.
  - 공휴일 정보 외부 API 연동: 공공데이터포털 특일 정보 API 를 요청 시점에 호출합니다 (현재 DB 캐시 없음 — 실패 시 `HOLIDAY_DATA_UNAVAILABLE 503` 으로 명시적 재시도 유도).
  - Excel 리포트 다운로드: 직원명·부서명·연장/휴일 근무 시간·수당이 담긴 `.xlsx` 를 Apache POI 스트리밍으로 생성·다운로드합니다.
